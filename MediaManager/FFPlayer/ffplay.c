...
//        }
		...
		if(is->videocb)
		is->videocb(&vp->pic,vp->width,vp->height);
    }
}
...
//	if(is->playcb)
//	{
//		is->playcb(is,0,get_master_clock(is),is->duration);
//	}
...
static void alloc_picture(void *opaque)
{
	...
	avpicture_alloc(&vp->pic, PIX_FMT_RGB24, is->video_st->codec->width, is->video_st->codec->height);
...
static int queue_picture(VideoState *is, AVFrame *src_frame, double pts1, int64_t pos)
{
	...
	if(!vp->pic.data[0] ||  vp->reallocate ||
#if CONFIG_AVFILTER
        vp->width  != is->out_video_filter->inputs[0]->w ||
        vp->height != is->out_video_filter->inputs[0]->h) {
#else
...
static void sdl_audio_callback(void *opaque, Uint8 *stream, int len)
{
	...
	if(!is->paused && is->playcb)
	{
		is->playcb(is,1,get_master_clock(is),is->duration);
	}
}
...
static int read_thread(void *arg)
{
	...
	is->duration = ic->duration / AV_TIME_BASE;
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat,pfn_playcallback playcb)
{
    VideoState *is;
...
void *eventloop_thread(void *argv)
{
	VideoState *is = (VideoState*)argv;
     event_loop(is);
	return 0;
}
...
//    }
	...
	//fs_screen_width = screenWidth;
	//fs_screen_height = screenHeight;
...
int startplay(VideoState *is)
{
	...
	is->loop_tid = SDL_CreateThread(eventloop_thread, is);
	if(!is->loop_tid)
	{
		av_free(is);
        return 1;
...
{
		do_exit((VideoState*)context);
	
}
	
void seek_newtime(void *context,double second)
{
	VideoState *cur_stream = context;
	double incr, pos, frac;
	incr = second;
//	if (seek_by_bytes) {
//		if (cur_stream->video_stream >= 0 && cur_stream->video_current_pos >= 0) {
