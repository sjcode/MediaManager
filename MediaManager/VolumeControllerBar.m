...
NSString * VolumeControllerBarValueDidChangeNotify = @"VolumeControllerBarValueDidChange";
@implementation VolumeControllerBar

- (void)awakeFromNib
{
	progressImage = [NSImage imageNamed:@"bg_volume_progress"];
	thumbImage = [NSImage imageNamed:@"bg_volume_thumb"];
...
- (void)drawRect:(NSRect)dirtyRect {
	...
	if(isMouseEnter)
	{
		...
		if(thumbRc.size.width < knobImage.size.width)
		{
			...
			knobRect = NSMakeRect(thumbRc.size.width-knobImage.size.width/2, 2, knobImage.size.width, knobImage.size.height);
		}
	...
	if (isMouseDown) {
		...
		if (diff>0.08) {  
			[[NSNotificationCenter defaultCenter]
			 postNotificationName:VolumeControllerBarValueDidChangeNotify
			 object:self
			 userInfo:[NSDictionary 
					   dictionaryWithObject:[NSNumber numberWithDouble:theValue]
					   forKey:@"VolumeBarValue"]];
